...
class new_rights extends linkBd{
		...
		function updete_admin(){
			...
			$check_Admin_In_Db = mysqli_query($link, "SELECT * FROM `user_acounts` WHERE email = '$new_adm' ");
			$admin = mysqli_fetch_assoc($check_Admin_In_Db);
			$admin = $admin['email'];
			if($admin){

				$result = mysqli_query($link, "UPDATE `user_rights` SET  `admin` = '1' , user = '0' WHERE `user_name` = '$admin' ");
			}
			else{
		...
		function delete(){
			...
			$check_Admin_In_Db = mysqli_query($link, "SELECT * FROM `user_acounts` 
